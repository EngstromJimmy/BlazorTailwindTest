...
## The problem?
Back in the day when i learnt CSS, it was a big thing not to repeat yourself. you shoul have all the styles in one central place.
...
In Blazor we can do that by adding a CSS file next to the component naming if the same wat and adding .css
...
Good: there is nothing predefined I have to take contious choises for everything we need.
Bad:I have to take contious choises for everything we need.
Bootstrap gets alot of hate because "all sites look the same", but you can also see it as you get help to get started quickly.

Tailwind takes the classes that we use in our documents and creates a CSS that only contains the thing we need.
Hense removing the bloat compared to Bootstrap.

One of the problemts with Tailwind is that it has micro classes, classes that only affect one of few things.
That means that a bootstrap button that woulf look like this:
``` html
...
```
Disclamer: I converted Boostrap to Tailwind using a online tool, there are defenetly better ways of converting from Bootstrap. But this gives us a feeling for what the problem is.
...
This is also the prefered way of doing styling, we could group everything into a .button-class but that is not the preferred way in Tailwind.
The idea is to style components seperatly in Angular and React for example.
...
In this blog post I aam going to try to solve two problems.
1. Making sure everyone on the team is runing the same version of the CLI
2. Make sure Blazor isolated CSS works
3. Not adding all the styles to the class tag

### Downloading the CLI
Instead of having all the members in the team downloading and managing their Tailwind CLI we can use Visual Studio to download and install it for us automatically.
In our csproj file we can simply add the version we want to use and let Visual studio manage the versioning.
Add the following in our csproj
```html
<PropertyGroup>
...
1. Triggers Tailswind to generate files on Build
2. Triggers Tailwind to intercept and generate isolated css on save

In most cases this should be all we need, but sometimes we want to change the CSS without having to build the project and that can still be done using the CLI from commandline.
...
Besides this, we need to setup our project for Tailwind.
...
This should support both Blazor server as well ass Blazor WebAssembly.
...
### Semantic CSS?
Now when everything it set up we can put our Tailwind styles inside our components by using isolated CSS.
...
We don't need to add any classes in the markup it self but instead have the CSS inside the isolated CSS file.

No NPM  
No manually downloading CLI  
No bloated markup  

I'll be honest I am still not a fan, but I think that since we can remove some of my painpoints perhaps one day I will start working with it =)
It feels a bit like I am going out of my way to remove the dependency on NPM, and if you don't mind NPM you should defenetöy continue to use it.
